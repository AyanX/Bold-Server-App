...
const DashboardView: React.FC = () => {
  ...
  const [isCategoryPreviewModalOpen, setIsCategoryPreviewModalOpen] =
    useState(false);
  const [isCampaignPreviewModalOpen, setIsCampaignPreviewModalOpen] =
    useState(false);
    ...
    const fetchData = async () => {
      ...
      try {
        const [
          ...
          await api.settings.sideBarCollapsed(),
        ]);
        ...
        setIsSidebarOpen(fetchedSideBarCollapsed.data.collapsed);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "ads":
        return <DashboardAds ads={ads} setAds={setAds} />;
      case "analytics":
        return (
          <DashboardAnalytics data={analyticsData || {}} logs={logs || []} />
        );
      default:
  ...
  const processPreviewContent = (content: string) => {
    ...
    return rawParagraphs.map((p, idx) => {
      ...
      if (processed.startsWith("## "))
        return (
          <h2 key={idx} className="text-2xl font-bold text-[#001733] mt-8 mb-4">
            {processed.slice(3)}
          </h2>
        );
      if (processed.startsWith("### "))
        return (
          <h3 key={idx} className="text-xl font-bold text-[#001733] mt-6 mb-3">
            {processed.slice(4)}
          </h3>
        );
      ...
      if (processed.startsWith("> "))
        return (
          <blockquote
            key={idx}
            className="border-l-4 border-[#e5002b] pl-4 italic text-xl text-gray-700 my-8 font-serif"
          >
            {processed.slice(2)}
          </blockquote>
        );
      ...
      let htmlContent = processed
        ...
        .replace(
          /\[(.*?)\]\((.*?)\)/g,
          '<a href="#" class="text-[#e5002b] hover:underline">$1</a>',
        );

      return (
        <p
          key={idx}
          className="mb-6 text-lg leading-relaxed text-gray-800 font-light"
        ...
        <div className="flex-grow flex flex-col h-[calc(100vh-64px)] overflow-y-auto w-full">
          <main className="p-8 lg:p-16 flex-grow">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
              <div>
                <h2 className="text-3xl font-black capitalize text-[#001733]">
                  {activeTab}
                </h2>
                <p className="text-gray-400 text-sm mt-1">
                  Manage your {activeTab} and view performance metrics.
                </p>
              </div>
            </header>
          ...
          <div className="bg-white w-full max-w-5xl h-[90vh] rounded-sm shadow-2xl overflow-hidden flex flex-col">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <div className="flex items-center gap-3">
                ...
                <span className="px-2 py-0.5 bg-gray-200 text-gray-600 text-[10px] font-bold uppercase rounded-sm">
                  Read Mode
                </span>
              </div>
              <button
                onClick={() => setIsArticlePreviewModalOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-8 md:p-16 bg-white">
              <article className="max-w-3xl mx-auto">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <span className="text-xs font-bold uppercase text-[#e5002b] tracking-widest">
                    {selectedArticle.category}
                  </span>
                </div>

                <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold text-[#001733] mb-8 leading-[1.1]">
                ...
                {selectedArticle.image && (
                  <figure className="mb-10">
                    ...
                    <figcaption className="mt-3 text-xs text-gray-400 font-medium italic border-l-2 border-gray-200 pl-4">
                      Featured Image
                    </figcaption>
                  </figure>
                )}
                ...
                <div className="prose prose-xl max-w-none">
                  <p className="text-2xl font-serif italic text-gray-500 mb-10 leading-relaxed border-l-4 border-[#001733] pl-8">
