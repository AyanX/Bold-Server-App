...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({ onTabChange, onMenuClick }) => {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 pl-4 border-l border-gray-100 group cursor-pointer relative">
          <div className="text-right hidden sm:block">
            <span className="block text-[10px] font-black uppercase tracking-widest text-[#001733]">{userEmail.split('@')[0]}</span>
            <span className="block text-[8px] font-bold text-gray-400 uppercase tracking-tighter">System Administrator</span>
          </div>
          <div className="w-10 h-10 rounded-full bg-[#001733] flex items-center justify-center text-white font-bold text-xs overflow-hidden">
             <img src="https://i.pravatar.cc/150?u=admin" className="w-full h-full object-cover" alt="Profile" />
          </div>
          ...
          <div className="absolute top-full right-0 mt-2 w-48 bg-white border border-gray-100 shadow-xl opacity-0 group-hover:opacity-100 transition-all invisible group-hover:visible z-50">
            <div className="p-4 border-b border-gray-50">
              ...
              <span className="block text-xs font-bold text-gray-800 truncate">{userEmail}</span>
            </div>
            <ul className="py-2">
              ...
              <li><button onClick={() => onTabChange?.('logs')} className="w-full text-left px-4 py-2 text-xs font-medium text-gray-600 hover:bg-gray-50 hover:text-[#001733]">System Logs</button></li>
