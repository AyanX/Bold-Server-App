            ...
            {growthData.map((item: any, i: number) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col justify-end group relative">
                  <div
                    className="w-full bg-[#001733] rounded-t-sm transition-all duration-500 group-hover:bg-[#e5002b]"
                    style={{ height: `${height}%` }}
                  >
          ...
          <div className="flex-grow overflow-y-auto max-h-[300px] p-4 space-y-4">
            {logs.map((log, i) => (
              <div key={i} className="flex gap-3 text-xs">
                <div className={`w-2 h-2 mt-1 rounded-full flex-shrink-0 ${log.level === 'error' ? 'bg-red-500' : 'bg-blue-500'}`}></div>
                <div>
                  <p className="font-bold text-gray-700">{log.action}</p>
                  <p className="text-gray-400">{log.user} • {log.timestamp || log.time}</p>
                </div>
              </div>
            ))}
            {logs.length === 0 && (
              <p className="text-center text-gray-400 text-sm italic py-10">No recent activity.</p>
            )}
          </div>
        </div>
    </div>
